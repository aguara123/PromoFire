...
import org.activity.promofire.adapter.AdapterServicios;
...
public class ActivitySearch extends AppCompatActivity {
    ...
    // activity transition
    public static void navigate(Activity activity, Categoria category) {
        Intent i = new Intent(activity, ActivitySearch.class);
        i.putExtra(EXTRA_CATEGORY_ID, category.idCategoria);
        i.putExtra(EXTRA_CATEGORY_NAME, category.categoria);
    ...
    private AdapterServicios adapterServicio;
    private ImageButton bt_clear;
    private View parent_view;
    private SwipeRefreshLayout swipe_refresh;
    private Call<CallBackPromocion> callbackCall = null;
    ...
    private void initComponent() {
        ...
        adapterServicio = new AdapterServicios(this, recyclerView, new ArrayList<Servicio>());
        recyclerView.setAdapter(adapterServicio);
        adapterServicio.setOnItemClickListener(new AdapterServicios.OnItemClickListener() {
            @Override
            public void onItemClick(View v, Servicio obj, int pos) {
                ActivityProductDetails.navigate(ActivitySearch.this, obj.idServicio, false);
            }
        });

        // detect when scroll reach bottom
        adapterServicio.setOnLoadMoreListener(new AdapterServicios.OnLoadMoreListener() {
            @Override
            public void onLoadMore(int current_page) {
                if (post_total > adapterServicio.getItemCount() && current_page != 0) {
                    int next_page = current_page + 1;
                    requestAction(next_page);
                } else {
                    adapterServicio.setLoaded();
                }
            }
        });
    ...
    private void requestAction(final int page_no) {
        ...
        if (page_no == 1) {
            ...
            adapterServicio.setLoading();
        }

        // analytics track
        PromoFireApp.getInstance().saveCustomLogEvent("SEARCH_PRODUCT", "keyword", query);
    ...
    private void requestListProduct(final int page_no) {
        ...
        callbackCall = api.getListPromo(page_no, Constant.PRODUCT_PER_REQUEST, query, category_id);
        callbackCall.enqueue(new Callback<CallBackPromocion>() {
            @Override
            public void onResponse(Call<CallBackPromocion> call, Response<CallBackPromocion> response) {
                ...
                if (resp != null && resp.status.equals("success")) {
                    post_total = resp.servicioList.size();
                    displayApiResult(resp.servicioList);
                } else {
    ...
    private void displayApiResult(final List<Servicio> items) {
        adapterServicio.insertData(items);
        swipeProgress(false);
