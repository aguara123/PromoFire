...
import org.activity.promofire.entity.BuyerProfile;
...
public class SharedPref {
    ...
    public AppVersion setAppVersionData(AppVersion appVersion) {
        if (appVersion == null) return null;
        String json = new Gson().toJson(appVersion, AppVersion.class);
        sharedPreferences.edit().putString(INFO_DATA, json).apply();
        return getAppVersionData();
    }

    public void clearAppVersionData() {
    ...
    public AppVersion getAppVersionData() {
        String data = sharedPreferences.getString(INFO_DATA, null);
        if (data == null) return null;
        return new Gson().fromJson(data, AppVersion.class);
    }
    ...
    public void setLoginInfo(JSONObject jsonObject) {
        ...
        sharedPreferences.edit().putString("nombre", jsonObject.optString("nombreApellido")).apply();
        sharedPreferences.edit().putString("email", jsonObject.optString("email")).apply();
        sharedPreferences.edit().putString("idRedSocial", jsonObject.optString("idRedSocial")).apply();
